...
The latest Stable Version is [1.2.0](https://github.com/Flowduino/ESPressio-Threads/releases/tag/1.2.0).
...
```ini
lib_deps =
    flowduino/ESPressio-Threads@^1.2.0
```
...
* `WithSharedReadLock()` invokes a callback taking `const T&`. `ReadWriteMutex` uses a shared lock for this method, allowing concurrent immutable readers.
* `TryWithSharedReadLock()` is the non-blocking counterpart to `WithSharedReadLock()` and returns whether the shared lock was acquired and the callback executed.
* `ReleaseReadLock()` releases a Lock acquired by `IsLockedRead()`.
* `ReleaseWriteLock()` releases a Lock acquired by `IsLockedWrite()`.
* `ReleaseLock()` remains available as a backward-compatible alias for `ReleaseReadLock()`.
